...
4. **Verify before exec** -- invoke `spawnSync('gh', ['attestation', 'verify', file, '--owner', 'zantarix', '--cert-identity', `https://github.com/zantarix/cursus/.github/workflows/release-artifacts.yml@refs/tags/cursus@${version}`])` against the binary at the target path. The `--cert-identity` URI pins both the workflow file path and the specific release tag ref, so an artifact built from the same workflow but a different version tag cannot pass verification. This matches the trust model already used by the cursus npm package's `verifyAttestation` function, which pins `certificateIdentityURI` to the same per-version URI. This step runs on **every** invocation, including cache hits -- the cache shortens the network path only, never the trust path. A verification failure after a fresh download (not a cache hit) is unrecoverable: delete the file and hard-fail with a clear error surfacing `gh`'s stderr. A verification failure against a cached file (poison recovery path) deletes the cached binary and falls through to the download path above; a subsequent verification failure after the re-download is then unrecoverable.
...
- **`version-file`** -- a path to an existing developer-tooling file in the consumer repository that already pins cursus for local development. The action shall detect the file format from its filename and parse the appropriate field:
  - `Cargo.toml`: attempt resolution in this order:
    1. `[package.metadata.bin.cursus-bin].version` -- the cargo-bin declaration (keyed as `cursus-bin`) for a per-package project.
    2. `[workspace.metadata.bin.cursus-bin].version` -- the cargo-bin declaration (keyed as `cursus-bin`) for a workspace root.
...
The trust model for the cursus binary the action installs is identical to the model established by [Cursus ADR-049](https://github.com/zantarix/cursus/blob/main/docs/adr/049-signed-release-artifacts.md) and [Cursus ADR-051](https://github.com/zantarix/cursus/blob/main/docs/adr/051-bundle-sigstore-deps-via-workspace-removal.md): identity-pinned Sigstore verification, hard-fail on any failure, no checksum-file fallback. The verifier is `gh attestation verify` -- the GitHub-audited CLI -- not a re-implementation. The `--cert-identity` flag pins the full certificate identity URI -- both the expected workflow file path and the specific release tag ref (`@refs/tags/cursus@${version}`) -- so that only an artifact produced from exactly that tagged release of the canonical cursus workflow can pass; the `--owner` flag pins the expected repository owner. Any artifact whose attestation does not satisfy both constraints shall be rejected. This is the same identity pin the cursus npm package's `verifyAttestation` function applies, ensuring the action and the npm postinstall enforce the same per-version trust anchor.
...
- The hard pin on `--cert-identity https://github.com/zantarix/cursus/.github/workflows/release-artifacts.yml@refs/tags/cursus@${version}` couples the action's identity policy to both that specific workflow filename in the cursus repository and the `cursus@<version>` tag scheme. If cursus ever renames or moves that workflow, or changes its tag-naming convention, the action must be released with a new identity expectation, and consumers pinning by SHA will need to bump. This is the same coupling [Cursus ADR-049](https://github.com/zantarix/cursus/blob/main/docs/adr/049-signed-release-artifacts.md) acknowledges between cursus's npm postinstall and its workflow path and tag scheme.
...
- Pinning consumer workflows by SHA is a recommendation, not an enforcement. The recommendation in `README.md` is the strongest enforcement mechanism this ADR contemplates; tooling-level enforcement (for example, requiring SHA pins via `actions: read` policy) is a consumer-side choice outside the scope of this action.
...
### 2026-05-25: Bundle verification supersedes attestations-API discovery for cursus >= 0.9.0

The "Action steps" (step 4), "Consumer workflow permissions", and "Binary trust model" sections describe verification as always performed via `gh attestation verify` discovering the Sigstore bundle through the GitHub attestations API, and therefore require consumers to grant `attestations: read`. For resolved cursus versions >= 0.9.0 this is now incorrect: [ADR-004](004-bundle-asset-verification.md) downloads the `<binary>.sigstore.json` bundle as a Release asset and verifies it offline with `gh ... --bundle`, so no attestations-API call is made and `attestations: read` is not required on that path. The attestations-API discovery path described here is retained only for pre-0.9.0 pins; consequently the "attestations API outage will hard-fail every CI job" negative consequence no longer holds for bundle-publishing versions. The trust model itself -- the pinned SAN, owner, OIDC trust root, and hard-fail philosophy -- is unchanged; only the bundle's transport is amended.
