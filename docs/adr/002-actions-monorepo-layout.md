...
**2026-05-02** -- [ADR-003](003-cursus-release-management.md) adopts cursus as the release-management tool for this repository and expands the role of the root `package.json`. Beyond carrying dev tooling, the root `package.json` is now also the single cursus-tracked package: cursus's npm driver enumerates only the root `@zantarix/actions` package (because no `workspaces` field is declared, per this ADR), so the root manifest's `version` field is the source of truth for the shared `vX.Y.Z` tag stream. The `setup-cursus/package.json` `version` field is decorative under that enumeration and is held at `0.0.0`. The "independent npm projects, not a workspace" rule from this ADR is unchanged; [ADR-003](003-cursus-release-management.md) relies on it.
